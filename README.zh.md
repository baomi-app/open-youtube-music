...
- **无缝单页内网页跳转**：底部控制栏上的歌手名、专辑名均采用交互式 SwiftUI 按钮及红色滑线悬停动效。点击即可通过 WebKit 桥接直接调用网页内的 Polymer 路由进行无刷新跳转，提供无缝的 App 内浏览体验，免去整页重新加载的卡顿。
- **极速流畅的进度条拖拽**：时间轴进度条支持直接拖拽定位，在拖拽期间自动挂起播放器时间回传以防进度条抖动挣扎，提供丝滑的拖拽寻道体验。
- **UserDefaults 自动持久化**：应用在启动时会自动读取、在修改时会自动保存您的歌词缩放比例、侧边栏歌词开启状态、桌面歌词秀开启状态等个性化配置，免去每次启动重复设置的烦恼。
- **原生内置广告拦截器**：在 WebKit 核心中以极低的 CPU 消耗直接部署声明式 `WKContentRuleList` 过滤规则，原生拦截广告域名、追踪器和网页广告组件。
